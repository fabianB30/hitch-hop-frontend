/**
 * Componente TopCancelaciones
 * ---------------------------
 * Muestra los 5 conductores con mayor cantidad de cancelaciones de viajes.
 *
...
/**
 * Interfaz que representa a un conductor con cancelaciones registradas.
 */
interface CancelledDriver {
  driverId: string;
  email: string;
  name: string;
...
/**
 * Componente funcional que consulta y muestra los conductores con más cancelaciones.
 */
export default function TopCancelaciones() {
  const [drivers, setDrivers] = useState<CancelledDriver[]>([]);

  // Hook para obtener los datos al cargar el componente
  useEffect(() => {
    const fetchData = async () => {
      const result = await queryTopDriversWithMostCancellations();
      console.log("Conductores con más cancelaciones:", result);
      if (Array.isArray(result?.data)) {
        setDrivers(result.data.slice(0, 5)); // Limita a los 5 primeros
      }
    };
    fetchData();
  }, []);
    ...
    <div className="p-6">
      ...
      >
          ...
          <table className="w-full table-fixed border-collapse">
            <thead className="bg-[#FFC750AA] text-[#262626] font-exo text-[16px] font-semibold">
              <tr className="h-[58px]">
                ...
                <th className="px-6 text-left truncate">Cancelaciones</th>
              </tr>
            </thead>
            <tbody>
              {drivers.length > 0 ? (
                drivers.map((driver) => (
                  ...
                  >
                    <td className="px-6 truncate whitespace-nowrap" title={driver.email}>
                      {driver.email}
                    </td>
                    <td className="px-6 truncate whitespace-nowrap" title={driver.name}>
                      {driver.name}
                    </td>
                    <td className="px-6">{driver.cancelledCount}</td>
                  </tr>
                ))
              ) : (
                <tr>
        ...
        <div className="mt-4 h-[46px] flex items-center justify-center text-black font-exo text-[20px] font-semibold bg-[#FFC750AA] rounded-md">
          Top conductores con más cancelaciones
        </div>
      </div>

      {/* Cantidad mostrada */}
      <div className="mt-2 text-[#171717] font-exo text-[20px] font-semibold">
        Mostrando top {drivers.length} de {drivers.length} conductores con cancelaciones
      </div>
    </div>
  );
}
